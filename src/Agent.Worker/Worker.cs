...
namespace Microsoft.VisualStudio.Services.Agent.Worker
{
    ...
    public sealed class Worker : AgentService, IWorker
    {
        ...
        private void AddUserSuppliedSecret(String secret)
        {
            ...
            HostContext.SecretMasker.AddValue(secret, "AddUserSuppliedSecret");
            ...
            foreach (var quoteChar in _quoteLikeChars)
            {
                ...
                {
                    HostContext.SecretMasker.AddValue(secret.Trim(quoteChar), "AddUserSuppliedSecretQuoteChar");
                }
            }
        }
        ...
        private void InitializeSecretMasker(Pipelines.AgentJobRequestMessage message)
        {
            ...
            foreach (MaskHint maskHint in (message.MaskHints ?? new List<MaskHint>()))
            {
                ...
                {
                    HostContext.SecretMasker.AddRegex(maskHint.Value, "Mask hints - regex");
                    ...
                    HostContext.SecretMasker.AddValue(maskHint.Value, "mask hints - value");
                }
                else
                {
            ...
            {
                foreach (var pairOb in endpoint.Authorization?.Parameters)
                {
                    if (!string.IsNullOrEmpty(pairOb.Value))
                    {
                        HostContext.SecretMasker.AddValue(pairOb.Value, pairOb.Key);
                    }
                }
            }
            ...
            foreach (SecureFile file in message.Resources.SecureFiles ?? new List<SecureFile>())
            {
                ...
                {
                    HostContext.SecretMasker.AddValue(file.Ticket, "file.Ticket");
                }
            }
        }
